    ...
    validateForm.validateSignup({
      ...
      models.User.findOne({where: {email: req.body.email}}).then(user => {
        ...
        models.User.findOne({where: {name: req.body.name}}).then(user => {
          if(user) return res.status(409).json({success: false, response: 'Full name already in use.'});
  ...
  authenticate: (req, res) => {
    validateForm.validateLogin({
      ...
      models.User.findOne({where: {email: req.body.email.trim()}}).then(user => {
        ...
        return res.status(200).json({success: true, response: 'Login confirmation link has been sent to your email address, please use it to access your account.'})
      })
    })
  },
  signin: (req, res) => {
          ...
          if(!err) {
            if(result === 1) {
              redisClient.get('twoFactorToken.'+user.email,function(err,result){
                if(result !== token+'-'+user.id) return res.render('auth/2fa', {message: 'Incorrect login confirmation link.'})
                redisClient.del('twoFactorToken.'+user.email)
              ...
              return res.render('auth/2fa', {message: 'Login Confirmation link expired.'})
            }
          }
        });
      });
